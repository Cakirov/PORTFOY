 ...
 *
 * Every diagram also takes an `animate` flag: `false` inside the mobile
 * project carousel, where these compositions are clipped by the horizontal
 * `overflow-x-auto` track (see ProjectGrid/NodeGraphic) — a `whileInView`
 * trigger there only fires once swiped into view (or, depending on the
 * browser, may not reliably fire at all for elements nested this deep
 * inside a scaled, clipped SVG). `false` renders every element already at
 * its "drawn in" end state immediately instead of waiting on a viewport
 * intersection that may never resolve.
 */

function drawIn(index = 0, duration = 0.9, animate = true) {
  const transition = { duration, ease: EASE_STANDARD, delay: 0.1 + index * 0.12 };
  return {
...
function popIn(index = 0, animate = true) {
  const transition = { duration: 0.45, ease: EASE_STANDARD, delay: 0.25 + index * 0.1 };
  return {
    initial: { opacity: 0, scale: 0.8 } as const,
    whileInView: animate ? ({ opacity: 1, scale: 1 } as const) : undefined,
    viewport: animate ? ({ once: true, amount: 0.5 } as const) : undefined,
    animate: animate ? undefined : ({ opacity: 1, scale: 1 } as const),
    transition: animate ? transition : { duration: 0 },
  };
}

function fadeIn(delay = 0.1, duration = 1, animate = true) {
  const transition = { duration, ease: EASE_STANDARD, delay };
  return {
    initial: { opacity: 0 } as const,
    whileInView: animate ? ({ opacity: 1 } as const) : undefined,
    viewport: animate ? ({ once: true, amount: 0.5 } as const) : undefined,
    animate: animate ? undefined : ({ opacity: 1 } as const),
    transition: animate ? transition : { duration: 0 },
  };
}
...
/** Slow "radar ping" ring — expands and fades, repeating, around a hero element.
    Not viewport-gated (plays on a plain `animate` loop, not `whileInView`), so
    it isn't affected by the mobile-carousel visibility issue above. */
...
function NovaDiagram({ animate }: DiagramProps) {
  ...
  return (
    <>
      <g strokeWidth={1} className="stroke-border-strong">
        {traceY.map((y, i) => (
          <motion.line key={y} x1={24} y1={y} x2={376} y2={y} {...drawIn(i, 0.7, animate)} />
        ))}
      </g>
      <g strokeWidth={1.25} className="stroke-current">
        {converge.map((p, i) => (
          <motion.line key={i} x1={p.x} y1={p.y} x2={hub.x} y2={hub.y} {...drawIn(i + 4, 0.6, animate)} />
        ))}
      </g>
      {blips.map((b, i) => (
        <motion.circle
      ...
      <motion.circle cx={hub.x} cy={hub.y} r={7} className="fill-current" {...popIn(8, animate)} />
...
function AtlasDiagram({ animate }: DiagramProps) {
  const modules = [
    { label: "CATALOG", y: 56 },
    { label: "PAYMENT", y: 182 },
  ...
  return (
    <>
      {modules.map((m, i) => (
        <motion.g key={m.label} {...popIn(i, animate)}>
      ...
      <g className="stroke-border-strong" strokeWidth={1}>
        {modules.map((m, i) => (
          <motion.path key={m.label} d={`M130,${m.y + 18} H${busX}`} {...drawIn(i, 0.5, animate)} />
        ))}
        <motion.line x1={busX} y1={74} x2={busX} y2={326} {...drawIn(3, 0.7, animate)} />
      </g>
      <g className="stroke-current" strokeWidth={1.25}>
        <motion.path d={`M${busX},200 H${cartX + 20}`} {...drawIn(4, 0.6, animate)} />
      </g>
      <motion.g {...popIn(4, animate)} className="stroke-current" strokeWidth={1.5} fill="none">
...
function NeuralDiagram({ animate }: DiagramProps) {
  const docs = [
    { x: 30, y: 50 },
    { x: 30, y: 160 },
    { x: 30, y: 270 },
  ...
  return (
    <>
      {docs.map((d, i) => (
        <motion.g key={i} {...popIn(i, animate)} className="stroke-text-tertiary" strokeWidth={1} fill="none">
      ...
      <g className="stroke-border-strong" strokeWidth={1} strokeDasharray="4 3">
        {docs.map((d, i) => (
          <motion.path key={i} d={`M${d.x + 70},${d.y + 40} H${bubble.x}`} {...drawIn(i, 0.6, animate)} />
        ))}
      </g>
      <motion.g {...popIn(3, animate)} className="stroke-current" strokeWidth={1.25}>
...
function FleetwatchDiagram({ animate }: DiagramProps) {
  ...
  return (
    <>
      ...
      <motion.g {...popIn(4, animate)} className="stroke-current" strokeWidth={1.5}>
        {bars.map((h, i) => (
          <line key={i} x1={330 + i * 12} y1={330} x2={330 + i * 12} y2={330 - h * 4} />
        ))}
      </motion.g>
      <motion.text x={318} y={350} fontSize={9} fontFamily="var(--font-mono)" className="fill-text-tertiary" {...fadeIn(0.9, 1, animate)}>
        LIVE
      </motion.text>
    </>
  );
...
function HorizonDiagram({ animate }: DiagramProps) {
  const bars = [
  ...
  return (
    <>
      <motion.line x1={axisX} y1={30} x2={axisX} y2={350} className="stroke-border-strong" strokeWidth={1} {...drawIn(0, 0.8, animate)} />
      ...
      <motion.line
        ...
        strokeWidth={1.25}
        strokeDasharray="5 4"
        {...drawIn(1, 0.9, animate)}
      />
      <motion.text x={236} y={44} fontSize={9} fontFamily="var(--font-mono)" className="fill-current" {...fadeIn(1, 1, animate)}>
        NOW
      </motion.text>
    </>
  );
...
function ComposeDiagram({ animate }: DiagramProps) {
  return (
    <>
      <g className="stroke-border-strong" strokeWidth={1}>
        <motion.line x1={200} y1={30} x2={200} y2={370} {...drawIn(0, 0.6, animate)} />
        <motion.line x1={30} y1={200} x2={370} y2={200} {...drawIn(1, 0.6, animate)} />
        <rect x={30} y={30} width={340} height={340} fill="none" />
      </g>

      <motion.g {...popIn(0, animate)} className="stroke-current" strokeWidth={1.25} fill="none">
        <rect x={65} y={100} width={100} height={34} rx={6} />
      </motion.g>

      <motion.g {...popIn(1, animate)} className="stroke-current" strokeWidth={1.25} fill="none">
        <rect x={245} y={100} width={80} height={30} rx={15} />
        <circle cx={305} cy={115} r={10} className="fill-current" stroke="none" />
      </motion.g>

      <motion.g {...popIn(2, animate)} className="stroke-text-tertiary" strokeWidth={1.25} fill="none">
...
function AuthCoreDiagram({ animate }: DiagramProps) {
  ...
  return (
    <>
      <g className="stroke-border-strong" strokeWidth={1}>
        {satellites.map((s, i) => (
          <motion.line key={s.label} x1={s.x} y1={s.y} x2={bodyCx} y2={bodyCy} {...drawIn(i, 0.6, animate)} />
        ))}
      </g>
      ...
      <motion.g {...popIn(3, animate)} className="stroke-current">
...
function EchoNotesDiagram({ animate }: DiagramProps) {
  ...
  return (
    <>
      <g className="stroke-current" strokeWidth={2.5} strokeLinecap="round">
        {bars.map((h, i) => (
          <motion.line key={i} x1={30 + i * 16} y1={200 - h / 2} x2={30 + i * 16} y2={200 + h / 2} {...drawIn(i, 0.4, animate)} />
        ))}
      </g>
...
function LivingSystemsDiagram({ animate }: DiagramProps) {
  ...
  return (
    <>
      <motion.path d={bandPath} className="fill-current" stroke="none" fillOpacity={0.12} {...fadeIn(0.1, 1, animate)} />
      <motion.path d={curvePath} fill="none" className="stroke-current" strokeWidth={1.5} {...drawIn(0, 1.2, animate)} />
      {markers.map((m, i) => (
        <motion.circle key={i} cx={m.x} cy={m.y} r={4} className="fill-current" {...popIn(i + 1, animate)} />
      ))}
...
function WayfinderDiagram({ animate }: DiagramProps) {
  ...
  return (
    <>
      <g className="stroke-border-strong" strokeWidth={1} strokeDasharray="4 4">
        {sources.map((s, i) => (
          <motion.line key={i} x1={s.x} y1={s.y} x2={glass.cx} y2={glass.cy} {...drawIn(i, 0.7, animate)} />
        ))}
      </g>

      <motion.g {...popIn(0, animate)} className="stroke-text-tertiary" strokeWidth={1} fill="none">
        <path
          d={`M${sources[0].x - 25},${sources[0].y - 30} H${sources[0].x + 15} L${sources[0].x + 25},${sources[0].y - 20} V${sources[0].y + 20} H${sources[0].x - 25} Z`}
        />
      </motion.g>

      <motion.g {...popIn(1, animate)} className="stroke-text-tertiary" strokeWidth={1.5} fill="none">
        <path d={`M${sources[1].x - 15},${sources[1].y - 20} L${sources[1].x - 28},${sources[1].y} L${sources[1].x - 15},${sources[1].y + 20}`} />
        <path d={`M${sources[1].x + 15},${sources[1].y - 20} L${sources[1].x + 28},${sources[1].y} L${sources[1].x + 15},${sources[1].y + 20}`} />
      </motion.g>

      <motion.g {...popIn(2, animate)} className="stroke-text-tertiary" strokeWidth={1} fill="none">
        <rect x={sources[2].x - 30} y={sources[2].y - 18} width={60} height={36} rx={8} />
      </motion.g>

      <motion.g {...popIn(3, animate)} className="stroke-current" strokeWidth={1.75} fill="none">
