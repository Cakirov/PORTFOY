  ...
  return (
    <motion.div layoutId={`project-card-${project.slug}`} className="h-full">
      <button
        ...
        className="group crosshair-zone relative flex h-full w-full flex-col overflow-hidden border border-border-strong bg-bg-elevated text-left transition-colors duration-(--motion-normal) hover:border-accent"
      >
        ...
        <motion.div
          ...
          className="relative aspect-square w-full overflow-hidden border-b border-border-strong bg-panel-2"
        >
          <div className="absolute inset-0 p-5 opacity-90 transition-transform duration-(--motion-normal) group-hover:scale-[1.06]">
            <NodeGraphic
              slug={project.slug}
        ...
        <div className="flex flex-1 flex-col gap-2 p-5">
          ...
          <p className="text-body line-clamp-2 text-text-secondary">{project.shortDescription}</p>

          <div className="mt-auto flex flex-wrap gap-1.5 pt-2">
            {project.technologies.slice(0, 3).map((tech) => (
              <Tag key={tech}>{tech}</Tag>
            ))}
            {project.technologies.length > 3 ? <Tag>+{project.technologies.length - 3}</Tag> : null}
          </div>
